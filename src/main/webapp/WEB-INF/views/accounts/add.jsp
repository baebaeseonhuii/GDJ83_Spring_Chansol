    ...
    <div class="row justify-content-center mt-5">
      <div class="col-lg-5 col-md-6">
        <form class="row g-3" method="post" id="frm">
          <div class="col-md-12">
            <label for="bank_pw" class="form-label">ACCOUNT PW</label> <input type="password" class="form-control" name="bank_pw" id="bank_pw">
          </div>
          <div class="col-md-12">
            <label for="pwval" class="form-label">ACCOUNT PW VALIDATION</label> <input type="password" class="form-control" id="pwval">
          </div>
        </form>
			...
			smbtn.addEventListener("click",
					function() {
						if (bank_pw.value == "" || pwval.value == "") {
							alert("공백을 채워주세요");
						} else if (bank_pw.value*1.0 != bank_pw.value || bank_pw.length != 4){
							alert("4개의 숫자를 입력하세요");
						}
							else {
							if (bank_pw.value == pwval.value) {
